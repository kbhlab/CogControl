...
if (study == "2") { # add 0.05 to avoid the all-zeros distribution
  
    new_temp <- tibble(id = "fake_p", 
                       trial_name = c("pre-switch_9", "post-switch_1"), 
                       trial_type = c("pre-switch", "post-switch"), 
                       language = "Monolinguals", 
                       SamplesTotal = c(60, 60), 
                       SamplesInAOI = c(3, 3))
...
learning_model <- glmer(SamplesInAOI / SamplesTotal ~ language * trial_type + (1 | id), 
                        data = learning_data, 
                        family = binomial, 
                        weights = SamplesTotal, 
                        control = glmerControl(optimizer="bobyqa", 
