...
This script was by Hilay Killam and Rodrigo Dal Ben.
...
Please indicate which study (1a, 1b, 1c, 1d, or 2) and which sample (filtered or unfiltered) you want to analyze. For Study 2 - filtered sample, it is also possible to include `vocabulary` as predictor. 

```{r}
study <- "1a" # options: "1a", "1b", "1c", "1d", "2"
filtered <- TRUE # options: TRUE = "filtered" OR FALSE = "unfiltered"

# TRUE can only be selected if 
...
if (study == "1a" | study == "2"){
  ...
  if (vocabulary == FALSE){
    ...
    PRED_COL <- c("language", "age_group", "trial_type", "trial_num")
    
  } else if (study == "2" & filtered == TRUE & vocabulary == TRUE){
    
     PRED_COL <- c("language", "age_group", "cdi_tot_vocab_prod", "vocab_group", "vocab_centred", "vocab_scaled", "trial_type", "trial_num")
     
  } else {
    
    "Against logic, the code worked until now. Please revise the parameters you defined in the first code chunk; we didn't find a matching pattern"
...
time_series_full_trial <-
  ...
  mutate(aoi = factor(aoi, levels = c("circle", "target", "distractor")),
         ...
         xmin = case_when(Time == X1 & language == "Monolinguals" & aoi == "target" ~ ANTICIP_START_OFF, 
         TRUE ~ X1),
         xmax = case_when(Time == X1 & language == "Monolinguals" & aoi == "target" ~ ANTICIP_END_OFF, 
         TRUE ~ X1),
         ymin = case_when(Time == X1 & language == "Monolinguals" & aoi == "target" ~ Inf, 
         TRUE ~ 0),
         ymax = case_when(Time == X1 & language == "Monolinguals" & aoi == "target" ~ -Inf,
         TRUE ~ 0)) %>% 
