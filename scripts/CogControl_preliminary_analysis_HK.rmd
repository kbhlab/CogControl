...
```{r echo = FALSE}
#tobii_full_export <- read_tsv(here("data", "keepers_only.tsv")) 
tobii_full_cleaned <- tobii_full_export %>% 
  ...
  pivot_longer(`AOI[Target]Hit`:`AOI[Circle]Hit_35`, names_to = "AOI", values_to = "looking") %>%
  mutate(AOI = case_when(grepl("Target", AOI) ~ "aoi_target",
...
proportion_trials <- inner_join(tobii_full_cleaned, ms_list, by = "recording_name") %>% 
...
###PICK UP HERE### must go back and add in the columns to indicate trial start/end!
response_window <- subset_by_window(eyetracking_data,
                                    window_start_time = 15500,
                                    window_end_time = 21000, rezero = TRUE, remove = TRUE)
