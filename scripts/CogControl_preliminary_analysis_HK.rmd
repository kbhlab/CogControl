...
Load in and clean full, time-series data from Tobii. 
```{r echo = FALSE}
...
save(tobii_full_export, file = here("data", "tobii_full_export.Rda"))
 ...
 tobii_full_cleaned <- tobii_full_export %>%
   #head(n = 25000) %>% # uncomment this line to test code changes on a subset of the full dataset since running the whole thing takes ages!
   select(StudioTestName, RecordingName, RecordingDuration, MediaName, RecordingTimestamp, StudioEvent, FixationIndex, GazeEventType, GazeEventDuration, `AOI[Target]Hit`:`AOI[Circle]Hit_35`, `GazePointX (ADCSpx)`, `GazePointY (ADCSpx)`, PupilLeft, PupilRight, ValidityLeft, ValidityRight) %>%
   mutate(circle = rowSums(.[grep("Circle", names(.))], na.rm = TRUE),
          target = rowSums(.[grep("Target", names(.))], na.rm = TRUE),
          distractor = rowSums(.[grep("Distractor", names(.))], na.rm = TRUE)) %>%
   select(-(`AOI[Target]Hit`:`AOI[Circle]Hit_35`)) %>%
   # pivot_longer(circle:distractor, names_to = "AOI", values_to = "looking") %>%
   # mutate(AOI = case_when(grepl("Target", AOI) ~ "aoi_target", #these lines are redundant since rowsumming
   #         grepl("Circle", AOI) ~ "aoi_circle",
   #         grepl("Distractor", AOI) ~ "aoi_distractor")) %>%
   # filter(!is.na(looking)) %>%
   # pivot_wider(names_from = "AOI", values_from = "looking") %>%
   # mutate(looking = as.numeric(looking)) %>%
   rename(gaze_point_X = `GazePointX (ADCSpx)`, gaze_point_y = `GazePointY (ADCSpx)`, timestamp = RecordingTimestamp) %>%
   mutate(trial_num = parse_number(MediaName)) %>%
   mutate(trial_type = case_when(str_detect(MediaName, "Training") ~ "pre-switch",
                                 str_detect(MediaName, "Test") ~ "post-switch")) %>%
 ...
 colnames(tobii_full_cleaned) <- to_snake_case(colnames(tobii_full_cleaned))
...
proportion_trials <- proportion_trials %>%
  filter(!is.na(media_name)) #remove inexplicable line with no media name

eyetracking_data <- make_eyetrackingr_data(proportion_trials, 
                               participant_column = "recording_name",
                               trial_column = "trial_name",
                               time_column = "timestamp",
                               trackloss_column = "trackloss",
                               aoi_columns = c('aoi_target', 'aoi_circle', 'aoi_distractor'),
                               treat_non_aoi_looks_as_missing = TRUE)

###PICK UP HERE### 
response_window <- subset_by_window(eyetracking_data,
                                    window_start_col = "trial_start",
                                    window_end_col = "trial_end", rezero = TRUE, remove = TRUE)

time_series_Data <- make_time_sequence_data(eyetracking_data,
                                         time_bin_size = 200,
                                         predictor_columns = c("language", "age_group", "vocab_group", "trial_type"),
                                         aois = "aoi_target",
                                         summarize_by = "recording_name")
