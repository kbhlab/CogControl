...
```{r echo = FALSE}
# data_tobii_full_export <- read_tsv(here("data", "tobii_2019-12-18.tsv"),
#                                    col_types = cols(LocalTimeStamp = col_character()))
# 
# save(data_tobii_full_export, file = here("data", "data_tobii_full_export.Rda"))

load(here("data", "data_tobii_full_export.Rda"))

###----------------------------------------------------------------------CLEAN DATA----------------------------------------------------------------------------

 data_tobii_full_cleaned <- data_tobii_full_export %>%
   ...
   select(StudioTestName, RecordingName, RecordingDuration, MediaName, RecordingTimestamp, StudioEvent, FixationIndex, GazeEventType, GazeEventDuration, `AOI[Target]Hit`:`AOI[Circle]Hit_35`, `GazePointX (ADCSpx)`, `GazePointY (ADCSpx)`, ValidityLeft, ValidityRight) %>% # choose only needed columns
  ...
  mutate(MediaName = gsub(".wmv", "", MediaName)) %>% # drops the .wmv part.
   ...
   mutate(trial_unique = as.factor(str_c(RecordingName, MediaName, sep = "_"))) %>% #get a trial identifier that is unique to each participant, trial number, and trial type
   # group_by(RecordingName) %>% # don't think this rleid bit is actually needed
  ...
  mutate(gaze_data_length = trial_from_zero - lag(trial_from_zero, default = 0)) %>% #this just gives us an idea of how long there is between each consecutive timestamp--there are about 10 odd lengths of over 200 ms, but most are between 14 and 19 ms, as we would expect
  ungroup() %>%
...
save(data_tobii_full_cleaned, file = here("data", "data_tobii_full_cleaned.Rda"))

#load(here("data", "data_tobii_full_cleaned.Rda"))

###----------------------------------------------------------------------CREATE MERGED FULL DATA----------------------------------------------------------------------------  


data_full_merged <- inner_join(data_tobii_full_cleaned, ms_list, by = "recording_name") %>%
  mutate(recording_name = as.factor(recording_name))


###----------------------------------------------------------------------CREATE ANTICIPATION PERIOD DATA----------------------------------------------------------------------------  

data_anticipation <- data_full_merged %>%
...
data_full_trials <- data_full_merged %>%
...
###----------------------------------------------------------------------CREATE 7 MONTH OLD DATA----------------------------------------------------------------------------  

data_7_anticipation <- data_anticipation %>%
  filter(age_group == "7 months")

data_7_full_trials <- data_full_trials %>%
  filter(age_group == "7 months")


###----------------------------------------------------------------------CREATE 20 MONTH OLD DATA----------------------------------------------------------------------------  

data_20_anticipation <- data_anticipation %>%
  filter(age_group == "20 months")

data_20_full_trials <- data_full_trials %>%
  filter(age_group == "20 months")


proportion_trials <- data_full_merged %>% 
...
data_eyetracking_full <- make_eyetrackingr_data(data_full_trials, 
...
trackloss <- trackloss_analysis(data = data_eyetracking_full)

data_eyetracking_clean <- clean_by_trackloss(data = data_eyetracking_full, trial_prop_thresh = .25)


##Don't really need to do the below anymore, since my new code in the previous section already subsets the data by the anticipation period

# response_window_7 <- subset_by_window(data_7_eyetracking,
#                                     window_start_time = 2150,
#                                     window_end_time = 3150, rezero = TRUE, remove = TRUE)
# 
# response_window_20 <- subset_by_window(data_20_eyetracking,
#                                     window_start_time = 2150,
#                                     window_end_time = 3150, rezero = TRUE, remove = TRUE)


data_time_series <- make_time_sequence_data(data_eyetracking_clean,
                                         time_bin_size = 100,
...
time_series_summary <- data_time_series %>%
...
plot_full_trials <- time_series_summary %>%
  ggplot(aes(x = Time, y = prop_looking, color = aoi)) + 
  geom_line() +
  facet_grid(trial_num ~ trial_type) +
  annotate("rect", xmin = 2150, xmax = 3150, ymin = -Inf, ymax = Inf, fill = "red", alpha = 0.1)
  #geom_vline(xintercept = 2150, col = "red", lty = 2)
