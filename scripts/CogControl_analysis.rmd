...
load(here("data", "tobii_full_export.Rda"))
 ...
 tobii_full_cleaned <- tobii_full_export %>%
   ...
   rename(gaze_point_X = `GazePointX (ADCSpx)`, gaze_point_y = `GazePointY (ADCSpx)`, timestamp = `RecordingTimestamp`, order = StudioTestName) %>% #fix names
   ...
   mutate(aoi_target_x_left = case_when(order == "Order 1" & trial_type == "pre-switch" ~ 10,
                                        ...
                                        TRUE ~ NA),
          aoi_target_y_top = 270,
          aoi_target_x_length = 645,
          aoi_target_y_length = 650)
  
 tobii_full_cleaned <- tobii_full_cleaned %>% 
   mutate(aoi_target_x_right = aoi_target_x_left + aoi_target_x_length,
          aoi_target_y_bottom = aoi_target_y_length - aoi_target_y_top)
          
          

 colnames(tobii_full_cleaned) <- to_snake_case(colnames(tobii_full_cleaned))
 tobii_full_cleaned$media_name <- gsub(".wmv", "", tobii_full_cleaned$media_name) # Drops the .wmv part.
...
# Prepping AOI (area of interest) file
aoi$target_x.bottomright = aoi.KBH$eyes.x.topleft + aoi.KBH$eyes.x.length
aoi.KBH$eyes.y.bottomright = aoi.KBH$eyes.y.topleft + aoi.KBH$eyes.y.length
aoi.KBH$mouth.x.bottomright = aoi.KBH$mouth.x.topleft + aoi.KBH$mouth.x.length
aoi.KBH$mouth.y.bottomright = aoi.KBH$mouth.y.topleft + aoi.KBH$mouth.y.length
aoi.KBH$face.x.bottomright = aoi.KBH$face.x.topleft + aoi.KBH$face.x.length
aoi.KBH$face.y.bottomright = aoi.KBH$face.y.topleft + aoi.KBH$face.y.length
...
full_data_merged <- full_data_merged %>%
  ...
  group_by(recording_name, trial_unique) %>%
  ...
  mutate(remove_row = case_when(timestamp == 66398 & trial_unique == "CogControl20_S01_42270_Test_Target_Right_4.wmv" & is.na(validity_left) ~ "remove",
                                ...
                                TRUE ~ "keep"))%>% #this gets rid of 3 rows with duplicate timestamps, but inexplicably without the fixation gaze data, that are preventing the eyetrackingR code from running correctly
  filter(remove_row == "keep")
