...
## Read tobii data  ---  order 1 has target on left during pre-switch, 2 has target on right during pre-switch
tobii_data_order1 <- read_csv(here("data", "2019-10-24_CogControl_tobii_data_ORDER1.csv"), na = "-") %>% 
  mutate(order = "1") %>% 
  filter(X1 != "All Recordings") #remove summary row
...
## Merge tobii orders
tobii_data <- bind_rows(tobii_data_order1, tobii_data_order2) #combine tobii orders
rm(tobii_data_order1, tobii_data_order2) #don't need these independent objects anymore since we've just merged them
...
load(here("data", "data_tobii_full_export.Rda")) #full export data is saved as RDA file since the csv is too large for GitHub
...
full_trial <- subset_by_window(data_eyetracking_full,
                                     window_start_time = 0,
                                     window_end_time = 5000, rezero = TRUE, remove = TRUE)



data_time_series <- make_time_sequence_data(full_trial,
                                         time_bin_size = 100,
...
time_series_summary <- data_time_series %>%
  ...
  annotate("rect", xmin = 2200, xmax = 3200, ymin = -Inf, ymax = Inf, fill = "red", alpha = 0.07) +
  annotate("text", x = 2700, y = 0.9, label = "Anticipation \n Period", size = 2.5, color = "#927774") +
  ggtitle(age_group) +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Time from trial start (ms)")  +
  ylab("Proportion looking")))
  
time_series_summary$plot[[1]]
time_series_summary$plot[[2]]
