...
data_full_trials <- data_merged %>%
  ...
  mutate(vocab_group = ifelse(median_by_group > total_vocab_prod,
                              ...
                              "High")) %>%
  ungroup() %>%
  mutate(vocab_group = as.factor(vocab_group))# this is the full time series data, but only for infants that made it through to the final anticipation-period sample. Will be used for ploting time series data of whole trial
```
...
```{r eyetrackingR_plots, echo = FALSE}
...
data_eyetracking_clean <- clean_by_trackloss(data = data_eyetracking_antic, trial_prop_thresh = .5) #this eliminates all data from trials with 50% or more trackloss (different from above, where we elimintated babies who did not have at least 50% of looking in at least 5/9 trials in both conditions)
...
(time_series_full_trial$plot[[1]] / time_series_full_trial$plot[[2]])
...
(time_series_antic$plot_all[[1]] / time_series_antic$plot_all[[2]])
...
vocab_plot <- data_for_models %>%
  filter(age_group == "20 months") %>%
  ggplot(aes(x = trial_num, y = Prop, color = AOI, linetype = vocab_group)) + 
  geom_smooth(se = FALSE) +
  facet_grid(language ~ fct_rev(trial_type)) +
...
(vocab_plot | time_series_vocab) # pretty close to each other 
...
check <- data_for_models %>%
  ...
  group_by(age_group) %>%
  nest() %>%
  mutate(plot_check = map2(data, age_group, ~ ggplot(data = .x, aes(x = trial_num, y = Prop, color = AOI, linetype = language)) + 
  geom_smooth(se = FALSE) +
  facet_grid( ~ (trial_type)) +
  ggtitle(age_group) +
  theme(plot.title = element_text(hjust = 0.5)) +
...
(check$plot_check[[2]] / check$plot_check[[1]] | time_series_antic$plot_all[[1]] / time_series_antic$plot_all[[2]]) ## why are these slightly different?

  
## homeless code - need to put somewhere after anticipation period = filtered out to get milliseconds looking to target/distractor  
  
  # mutate(target_ms = target * (1000/60)) %>% #converts looks to milliseconds based on sampling rate of eyetracker
  # mutate(distractor_ms = distractor * (1000/60)) %>%  
  # mutate(circle_ms = circle * (1000/60)) %>%
  # group_by(trial_unique) %>%
  # mutate(target_ms_per_trial = sum(target_ms)) %>%
  # mutate(distractor_ms_per_trial = sum(distractor_ms)) %>%
  # mutate(circle_ms_per_trial = sum(circle_ms)) %>%
  # ungroup()
