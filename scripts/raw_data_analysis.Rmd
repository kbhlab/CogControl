...
data_tobii_cleaned <- data_tobii_full_export %>%
...
data_eyetracking_full_trials <- make_eyetrackingr_data(data_full_trials, 
                                ...
                                trial_column = "trial_name", #question: is this meant to distinguish between different orders? (so, pre-switch target left != pre-switch target right?)
...
time_series_antic$plot[[1]]
time_series_antic$plot[[2]]
...
model_data <- data_time_series_antic %>%
  ...
  mutate(time_scaled = Time / 100) #rescale time to help with large eigenvalue thing as done here: https://bit.ly/2O3tQne 
...
model_pre_7 <- glmer(Prop ~ language * time_scaled + (1 | recording_name), data = model_pre_7_data, family = "binomial")
...
model_pre_20 <- glmer(Prop ~ language * time_scaled + (1 | recording_name), data = model_pre_20_data, family = "binomial")

summary(model_pre_20)

plot(model_pre_20) # question: is this problematic?
