...
data_eyetracking_clean_nocircle <- make_eyetrackingr_data(data_full_trials, 
                                ...
                                time_column = "trial_from_zero",
                                trackloss_column = "trackloss",
                                aoi_columns = c('target', 'distractor'), # this will calculate prop looking in relation to all AOIs, not just target vs distractor. When I remove circle from here, the plot gets very wonky (I guess because of dividing by 0 in some cases?)
                                treat_non_aoi_looks_as_missing = TRUE) %>%
...
plot_ab_7 <- plot_ab$plot[[1]] 
plot_ab_20 <- plot_ab$plot[[2]] +
...
##glmer model
```{r across_trial_models, echo = FALSE}

###------------------------------------------------MODELS BY AGE, LANG, TRIAL TYPE---------------------------------------------  

models_by_language_trial_type <- data_for_models %>%
  #filter(trial_num > 1) %>% # get rid of trial one, where slopes are wildly different because infants have no data to make a prediction
...
emmeans(model1_20_pre, ~ trial_num_start_0 * language)
...
```{r across_time_models, echo = FALSE}

###--------------------------------------------------------------TIME BIN MODELS------------------------------------ 

data_tb <- data_time_series_antic %>%
  mutate(trial_num_start_0 = trial_num - 1) %>%
  filter(AOI == "target")

model_timebin <- data_tb %>%
  mutate(trial_num_start_0 = trial_num - 1) %>%
  mutate(trial_num_5_centre = trial_num - 5) %>%
  mutate(trial_num_factor = factor(trial_num)) %>%
  mutate(trial_num_decimal_centre = scale(trial_num, center = TRUE, scale = FALSE)) %>%
  ...
  group_by(age_group, trial_type) %>%
  nest() %>%
  mutate(model1 = map(data, ~ lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name), data = .))) 


tb_pre20<- model_timebin$model1[[1]] 
tb_post20<- model_timebin$model1[[2]]
tb_pre7<- model_timebin$model1[[3]]
tb_post7<- model_timebin$model1[[4]]
...
ggpredict(tb_pre7, c("trial_num_start_0", "language")) %>% plot()
ggpredict(tb_post7, c("trial_num_start_0", "language")) %>% plot()
ggpredict(tb_pre20, c("trial_num_start_0", "language")) %>% plot()
ggpredict(tb_post20, c("trial_num_start_0", "language")) %>% plot()

```

##Time Bin Models with Intercept for Trial Num - decent fit when plotted!

```{r models_with_intercept_for_trial_num}

blues <- c("#AFEDFD", "#86CBFD", "#75B4E9", "#659ED6", "#5488C3", "#4472B0", "#23458A", "#122F77", "#021964")
blues2 <- c("#79C9DC", "#6AB1C8", "#5B9AB4", "#4C83A0", "#3D6C8C", "#2E5478", "#1F3D64", "#102650", "#010F3C")

data_tb_7pre <- data_tb %>%
  filter(age_group == "7 months",
         trial_type == "pre-switch")

tb_pre7_1 <- lmer(Prop ~ language * TimeBin + (1 | recording_name), data = data_tb_7pre)
tb_pre7_2 <- lmer(Prop ~ language * trial_num_start_0 + (1 | recording_name), data = data_tb_7pre)
tb_pre7_3 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name), data = data_tb_7pre)
tb_pre7_4 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name), data = data_tb_7pre, REML = TRUE)
tb_pre7_5 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name) + (1 | trial_num_start_0), data = data_tb_7pre, REML = TRUE)

data_tb_7post <- data_tb %>%
  filter(age_group == "7 months",
         trial_type == "post-switch")

tb_post7_1 <- lmer(Prop ~ language * TimeBin + (1 | recording_name), data = data_tb_7post)
tb_post7_2 <- lmer(Prop ~ language * trial_num_start_0 + (1 | recording_name), data = data_tb_7post)
tb_post7_3 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name), data = data_tb_7post)
tb_post7_4 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name), data = data_tb_7post, REML = TRUE)
tb_post7_5 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name) + (1 | trial_num_start_0), data = data_tb_7post, REML = TRUE)

data_tb_20pre <- data_tb %>%
  filter(age_group == "20 months",
         trial_type == "pre-switch")

tb_pre20_1 <- lmer(Prop ~ language * TimeBin + (1 | recording_name), data = data_tb_20pre)
tb_pre20_2 <- lmer(Prop ~ language * trial_num_start_0 + (1 | recording_name), data = data_tb_20pre)
tb_pre20_3 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name), data = data_tb_20pre)
tb_pre20_4 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name), data = data_tb_20pre, REML = TRUE)
tb_pre20_5 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name) + (1 | trial_num_start_0), data = data_tb_20pre, REML = TRUE)

data_tb_20post <- data_tb %>%
  filter(age_group == "20 months",
         trial_type == "post-switch")

tb_post20_1 <- lmer(Prop ~ language * TimeBin + (1 | recording_name), data = data_tb_20post)
tb_post20_2 <- lmer(Prop ~ language * trial_num_start_0 + (1 | recording_name), data = data_tb_20post)
tb_post20_3 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name), data = data_tb_20post)
tb_post20_4 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name), data = data_tb_20post, REML = TRUE)
tb_post20_5 <- lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name) + (1 | trial_num_start_0), data = data_tb_20post, REML = TRUE)

labs(title = "Predicted values of proportion looking to target (20 months, test)", x = "Time in milliseconds during anticipation period", y = "Proportion looking to target", color = "Trial number") +
  scale_color_manual(values = blues, labels = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  scale_x_continuous(breaks = c(0, 2, 4), labels = c("0", "500", "1000")) +
  ylim(-.2, 1)


ggpredict(tb_pre7_4, c("trial_num_start_0", "language")) %>% plot()
ggpredict(tb_post7_int, c("trial_num_start_0", "language")) %>% plot()
ggpredict(tb_pre20_int, c("trial_num_start_0", "language")) %>% plot()
ggpredict(tb_post20_int, c("trial_num_start_0", "language")) %>% plot()

fanpre7 <- ggpredict(tb_pre7_4, terms = c("TimeBin", "trial_num_start_0", "language"), type = "re", ci.lvl = FALSE) %>% plot(colors = "trial_num_start_0", line.size = 1) +
  labs(title = "Predicted values of proportion looking to target (7 months, training)", x = "Time in milliseconds during anticipation period", y = "Proportion looking to target", color = "Trial number") +
  scale_color_manual(values = blues, labels = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  scale_x_continuous(breaks = c(0, 2, 4), labels = c("0", "500", "1000")) +
  ylim(-0.1, 1) +
  theme(legend.position = "none")

fanpost7 <- ggpredict(tb_post7_4, terms = c("TimeBin", "trial_num_start_0", "language"), type = "re", ci.lvl = FALSE) %>% plot(colors = "trial_num_start_0", line.size = 1) +
  labs(title = "Predicted values of proportion looking to target (7 months, test)", x = "Time in milliseconds during anticipation period", y = "Proportion looking to target", color = "Trial number") +
  scale_color_manual(values = blues, labels = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  scale_x_continuous(breaks = c(0, 2, 4), labels = c("0", "500", "1000")) +
  ylim(-0.1, 1)

fanpre20 <- ggpredict(tb_pre20_4, terms = c("TimeBin", "trial_num_start_0", "language"), type = "re", ci.lvl = FALSE) %>% plot(colors = "trial_num_start_0", line.size = 1) +
  labs(title = "Predicted values of proportion looking to target (20 months, training)", x = "Time in milliseconds during anticipation period", y = "Proportion looking to target", color = "Trial number") +
  scale_color_manual(values = blues, labels = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  scale_x_continuous(breaks = c(0, 2, 4), labels = c("0", "500", "1000")) +
  ylim(-0.1, 1) +
  theme(legend.position = "none")

fanpost20 <- ggpredict(tb_post20_4, terms = c("TimeBin", "trial_num_start_0", "language"), type = "re", ci.lvl = FALSE) %>% plot(colors = "trial_num_start_0", line.size = 1) +
  labs(title = "Predicted values of proportion looking to target (20 months, test)", x = "Time in milliseconds during anticipation period", y = "Proportion looking to target", color = "Trial number") +
  scale_color_manual(values = blues, labels = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  scale_x_continuous(breaks = c(0, 2, 4), labels = c("0", "500", "1000")) +
  ylim(-0.1, 1)
...
spreadpre7 <- ggpredict(tb_pre7_5, terms = c("TimeBin", "trial_num_start_0", "language"), type = "re", ci.lvl = FALSE) %>% plot(colors = "trial_num_start_0", line.size = 1) +
  labs(title = "Predicted values of proportion looking to target (7 months, training)", x = "Time in milliseconds during anticipation period", y = "Proportion looking to target", color = "Trial number") +
  scale_color_manual(values = blues, labels = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  scale_x_continuous(breaks = c(0, 2, 4), labels = c("0", "500", "1000")) +
  ylim(-.18, 1) +
...
spreadpost7 <- ggpredict(tb_post7_5, terms = c("TimeBin", "trial_num_start_0", "language"), type = "re", ci.lvl = FALSE) %>% plot(colors = "trial_num_start_0", line.size = 1) +
  labs(title = "Predicted values of proportion looking to target (7 months, test)", x = "Time in milliseconds during anticipation period", y = "Proportion looking to target", color = "Trial number") +
  scale_color_manual(values = blues, labels = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  scale_x_continuous(breaks = c(0, 2, 4), labels = c("0", "500", "1000")) +
  ylim(-.18, 1)

spreadpre20 <- ggpredict(tb_pre20_5, terms = c("TimeBin", "trial_num_start_0", "language"), type = "re", ci.lvl = FALSE) %>% plot(colors = "trial_num_start_0", line.size = 1) +
  labs(title = "Predicted values of proportion looking to target (20 months, training)", x = "Time in milliseconds during anticipation period", y = "Proportion looking to target", color = "Trial number") +
  scale_color_manual(values = blues, labels = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  scale_x_continuous(breaks = c(0, 2, 4), labels = c("0", "500", "1000")) +
  ylim(-.18, 1) +
...
spreadpost20 <- ggpredict(tb_post20_5, terms = c("TimeBin", "trial_num_start_0", "language"), type = "re", ci.lvl = FALSE) %>% plot(colors = "trial_num_start_0", line.size = 1) +
  labs(title = "Predicted values of proportion looking to target (20 months, test)", x = "Time in milliseconds during anticipation period", y = "Proportion looking to target", color = "Trial number") +
  scale_color_manual(values = blues, labels = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  scale_x_continuous(breaks = c(0, 2, 4), labels = c("0", "500", "1000")) +
  ylim(-.18, 1)

fan7 <- fanpre7 + plot_spacer() + fanpost7 + plot_layout(widths = c(5, .25, 5))
spread7 <- spreadpre7 + plot_spacer() + spreadpost7 + plot_layout(widths = c(5, .25, 5))
fan20 <- fanpre20 + plot_spacer() + fanpost20 + plot_layout(widths = c(5, .25, 5))
spread20 <- spreadpre20 + plot_spacer() + spreadpost20 + plot_layout(widths = c(5, .25, 5))
...
anova(tb_pre7_1, tb_pre7_3) # looks like having trial number improves fit
anova(tb_pre7_2, tb_pre7_3) # looks like having time bin really improves fit
anova(tb_pre7_4, tb_pre7_5, refit = FALSE) # having a random intercept for trial number does NOT improve fit

anova(tb_post7_1, tb_post7_3) # looks like having trial number improves fit
anova(tb_post7_2, tb_post7_3) # looks like having time bin improves fit
anova(tb_post7_4, tb_post7_5, refit = FALSE) # having a random intercept for trial number DOES improve fit

anova(tb_pre20_1, tb_pre20_3) # looks like having trial number slightly improves fit
anova(tb_pre20_2, tb_pre20_3) # looks like having time bin really improves fit
anova(tb_pre20_4, tb_pre20_5, refit = FALSE) # having a random intercept for trial number DOES improve fit

anova(tb_post20_1, tb_post20_3) # looks like having trial number improves fit
anova(tb_post20_2, tb_post20_3) # looks like having time bin improves fit
anova(tb_post20_4, tb_post20_5, refit = FALSE) # having a random intercept for trial number DOES improve fit
...
block_data <- data_eyetracking_clean %>%
  ...
  mutate(correct = case_when(looking_ms_target > (looking_ms_distractor + looking_ms_circle) ~ 1, 
                                   TRUE ~ 0),
         incorrect = case_when(looking_ms_target < (looking_ms_distractor + looking_ms_circle) ~ 1, 
                                     TRUE ~ 0),
...
```{r messy_notes}
block_data_nocircle <- data_eyetracking_clean_nocircle %>%
  make_time_window_data(aois = c("target", "distractor"),
                        predictor_columns = c("age_group", "language", "total_vocab_prod", "vocab_group", "trial_type", "trial_num")) %>%
  select(-Elog, -Weights, -LogitAdjusted, -ArcSin) %>%
  mutate(looking_ms = Prop * (SamplesTotal * (1000/60))) %>% #adds number of milliseconds spent looking at target for the trial
  pivot_wider(names_from = AOI, values_from = c(SamplesInAOI, Prop, looking_ms)) %>%
  mutate(total_looking_time = looking_ms_target + looking_ms_distractor) %>%
  ...
  summarise(num_trials_contributed = length(correct), 
            correct_anticipation = mean(correct, na.rm = T),
            total_anticipation = mean(total_anticipation)) %>%
  group_by(recording_name, trial_type) %>%
  mutate(n_blocks = length(unique(block_num)))


blocks_figure_circle <- block_data %>%
  filter(n_blocks == 3) %>%
  mutate(trial_type_labs = case_when(trial_type == "pre-switch" ~ "Training",
                                     trial_type == "post-switch" ~ "Test",
                                     TRUE ~ NA_character_)) %>%
  ...
  facet_grid(age_group ~ trial_type_labs) +
  scale_y_continuous(limits = c(0:1)) +
  theme_ggeffects() +
  theme(panel.border = element_rect(color = "lightgrey", fill = NA)) +
  scale_fill_manual(values = c("#23458A", # bilingual colour
                               "#75B4E9")) + # mono colour
  xlab("Block number") +
  ylab("Proportion of infants with \ncorrect anticipation")

blocks_figure_nocircle <- block_data_nocircle %>%
  ggplot(aes(x = block_num, y = correct_anticipation, fill = language)) +
  stat_summary(fun.y = "mean", geom = "bar", position = "dodge") +
  facet_grid(age_group ~ trial_type) +
  scale_y_continuous(limits = c(0:1)) +
...
(blocks_figure_circle | blocks_figure_nocircle)
...
anticipation_plots_nocircle <- data_eyetracking_clean_nocircle %>% 
  make_time_window_data(aois = "target",
                        predictor_columns = c("age_group", "language", "total_vocab_prod", "vocab_group", "trial_type", "trial_num")) %>%
  mutate(looking_ms = Prop * (SamplesTotal * (1000/60))) %>% 
  group_by(age_group) %>%
  nest() %>%
  mutate(plot = map2(data, age_group, ~ ggplot(data = .x, aes(x = trial_num, y = Prop)) + 
  geom_smooth(aes(color = language), se = FALSE) +
...
anticipation_plots_nocircle$plot[[2]]
```
