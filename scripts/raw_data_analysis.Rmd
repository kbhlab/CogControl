...
models_by_language_trial_type <- data_for_models %>%
  ...
  mutate(trial_num_start_0 = trial_num - 2) %>%
...
model_timebin <- data_gc %>%
  ...
  group_by(age_group, trial_type) %>%
  nest() %>%
  mutate(model1 = map(data, ~ lmer(Prop ~ language * TimeBin * trial_num_start_0 + (1 | recording_name) + (1 | trial_num), data = .))) 


data_gc_7pre <- data_gc %>%
  filter(age_group == "7 months",
         trial_type == "pre-switch")
lmer(Prop ~ language * TimeBin * trial_num + (1 | recording_name) + (1 | trial_num), data = data_gc_7pre) %>% summary()

data_gc_7post <- data_gc %>%
  filter(age_group == "7 months",
         trial_type == "post-switch")
lmer(Prop ~ language * TimeBin * trial_num + (1 | recording_name) + (1 | trial_num), data = data_gc_7post) %>% summary()

data_gc_20pre <- data_gc %>%
  filter(age_group == "20 months",
         trial_type == "pre-switch")
lmer(Prop ~ language * TimeBin * trial_num + (1 | recording_name) + (1 | trial_num), data = data_gc_20pre) %>% summary()

data_gc_20post <- data_gc %>%
  filter(age_group == "20 months",
         trial_type == "post-switch")
lmer(Prop ~ language * TimeBin * trial_num + (1 | recording_name) + (1 | trial_num), data = data_gc_20post) %>% summary()
