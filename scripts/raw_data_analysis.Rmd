...
#-----------------------MS lists for vocab sample

final_sample_mslist_vocab <- final_sample_mslist %>%
  filter(!is.na(cdi_tot_vocab_prod)) #this MS list is to be used for vocabulary models for 20-month-olds

unfiltered_sample_mslist_vocab <-ms_list_unfiltered %>%
  filter(!is.na(cdi_tot_vocab_prod))
...
#----------------Check pre-filtered sample counts
exclusions %>%
  mutate(excl_original = case_when(excl_reason == "inattentive" ~ "none",
                                   TRUE ~ excl_reason)) %>%
  count(age_group, language, excl_original) %>%
  arrange(age_group, desc(n))
...
#To make plots and models with unfiltered data, uncomment the following lines and change TL_PROP to 1

# data_full_trials <- data_full_trials_unfilterd %>%
#   mutate(vocab_group = NA_character_, #add these blank columns so that the next section runs
...
# ggsave("plot_full_trial_7_final.png", path = here("figures"), plot = time_series_full_trial$plot[[1]], device = "png", width = 9, height = 9, units = "in", dpi = 300)
# ggsave("plot_full_trial_20_final.png", path = here("figures"), plot = time_series_full_trial$plot[[2]], device = "png", width = 9, height = 9, units = "in", dpi = 300)
...
ggsave("plot_mono_7allpre.png", path = here("figures"), device = "png", width = 7, height = 10, units = "in",
   dpi = 300)
...
ggsave("plot_bi_7allpre.png", path = here("figures"), device = "png", width = 7, height = 10, units = "in",
   dpi = 300)
...
ggsave("plot_mono_all7post.png", path = here("figures"), device = "png", width = 7, height = 10, units = "in",
   dpi = 300)
...
ggsave("plot_bi_all7post.png", path = here("figures"), device = "png", width = 7, height = 10, units = "in",
   dpi = 300)
...
#full model
lmermodel_7pre <- lmer(Prop ~ language * time_bin_centred * trial_num_scaled + (1 | recording_name), data = model_data_7_pre, REML = FALSE)
#without trial number
lmermodel_7pre2 <- lmer(Prop ~ language * time_bin_centred + (1 | recording_name), data = model_data_7_pre, REML = FALSE)
#without time bin
...
glmermodel_7pre.2 <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * trial_num_scaled + (trial_num_scaled | recording_name), data = model_data_7_pre, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000))) # nearly everything is significant in the glmer model
...
anova(glmermodel_7pre, glmermodel_7pre.2) #model with random slopes is better, but we will go with the no slope model to match the 20-month-olds
...
#full model
lmermodel_7post <- lmer(Prop ~ language * time_bin_centred * trial_num_scaled + (1 | recording_name), data = model_data_7_post, REML = FALSE)
#without trial number
lmermodel_7post2 <- lmer(Prop ~ language * time_bin_centred + (1 | recording_name), data = model_data_7_post, REML = FALSE)
#without time bin
...
#glmer with random slope
glmermodel_7post.2 <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * trial_num_scaled + (trial_num_scaled | recording_name), data = model_data_7_post, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000))) # nearly everything is significant in the glmer model
tab_model(glmermodel_7post.2, show.re.var = FALSE, show.icc = FALSE)
...
##----------------Only useful for LMER models!!! Not GLMER
...
ggpredict(final_model_pre_7, terms = c("time_bin_centred", "trial_num_scaled", "language"), type = "re", ci.lvl = FALSE) %>% plot(colors = "trial_num_scaled", line.size = 1, connect.lines = TRUE) +
  labs(title = "Predicted values of proportion looking to target during the Training phase \nin Experiment 1a (7 months)", x = "Time in milliseconds during anticipation period", y = "Proportion looking to target", color = "Trial number") +
  scale_color_manual(values = blues, labels = c(2, 3, 4, 5, 6, 7, 8, 9)) +
...
#ggsave("plot_model_7_pre.png", path = here("figures"), device = "png", width = 7, height = 4, units = "in", dpi = 300)
...
#ggsave("plot_model_7_post.png", path = here("figures"), device = "png", width = 7, height = 4, units = "in", dpi = 300)
...
lmermodel_20pre_novocab <- lmer(Prop ~ language * time_bin_centred * trial_num_scaled + (1 | recording_name), data = model_data_20_pre, REML = FALSE)

lmermodel_20pre <- lmer(Prop ~ language * vocab_centred * time_bin_centred * trial_num_scaled + (1 | recording_name), data = model_data_20_pre, REML = FALSE)
...
glmermodel_20pre <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * vocab_scaled * trial_num_scaled + (trial_num_scaled | recording_name), data = model_data_20_pre, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

glmermodel_20pre.2 <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * trial_num_scaled + vocab_scaled * trial_num_scaled + vocab_scaled * language + (trial_num_scaled | recording_name), data = model_data_20_pre, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

glmermodel_20pre.3 <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * trial_num_scaled + vocab_scaled * trial_num_scaled + vocab_scaled * language + (1 | recording_name), data = model_data_20_pre, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

glmermodel_20prenovocab <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * trial_num_scaled + (1 | recording_name), data = model_data_20_pre, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

anova(glmermodel_20pre.2, glmermodel_20pre.3)

tab_model(glmermodel_20pre.3, show.re.var = FALSE, show.icc = FALSE)

###-------------------------------------------------------------------FINAL MODEL-------------

final_model_pre_20 <- glmermodel_20prenovocab

###-------------------------------------------------------------POST SWITCH 20--------------

lmermodel_20post_novocab <- lmer(Prop ~ language * time_bin_centred * trial_num_scaled + (1 | recording_name), data = model_data_20_post, REML = FALSE)

lmermodel_20post <- lmer(Prop ~ language * vocab_centred * time_bin_centred * trial_num_scaled + (1 | recording_name), data = model_data_20_post, REML = FALSE)
...
#This model is overfitting - let's remove the interaction between vocab and time bin
glmermodel_20post <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * vocab_scaled * trial_num_scaled + (trial_num_scaled | recording_name), data = model_data_20_post, weights = SamplesTotal, family = binomial, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

glmermodel_20postnovocab <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * trial_num_scaled + (trial_num_scaled | recording_name), data = model_data_20_post, weights = SamplesTotal, family = binomial, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

glmermodel_20post.2 <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * trial_num_scaled + vocab_scaled * trial_num_scaled + vocab_scaled * language + (trial_num_scaled | recording_name), data = model_data_20_post, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

glmermodel_20post.3 <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * trial_num_scaled + vocab_scaled + vocab_scaled:trial_num_scaled + vocab_scaled:language + (1 | recording_name), data = model_data_20_post, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

glmermodel_20postnovocab <- glmer(SamplesInAOI / SamplesTotal ~ language * time_bin_centred * trial_num_scaled + (1 | recording_name), data = model_data_20_post, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

anova(glmermodel_20post.2, glmermodel_20post.3, test = "Chisq")

tab_model(glmermodel_20post.3, show.re.var = TRUE, show.icc = TRUE)

###-------------------------------------------------------------------FINAL MODEL-------------

final_model_post_20 <- glmermodel_20postnovocab


###-------------------------------------------------------------ARE RESIDUALS NORMAL??--------------

##---------------This section is only really useful for the LMER models, not GLMER
...
ggsave("plot_model_20_pre_final.png", path = here("figures"), device = "png", width = 7, height = 7, units = "in",
   dpi = 300)
...
ggsave("plot_model_20_post_final.png", path = here("figures"), device = "png", width = 7, height = 7, units = "in",
   dpi = 300)
```
...
flipped_pre_20 <- glmer(SamplesInAOI / SamplesTotal ~ factor(language, levels = c("Bilinguals", "Monolinguals")) * time_bin_centred * trial_num_scaled + vocab_scaled + vocab_scaled:trial_num_scaled + vocab_scaled:language + (1 | recording_name), data = model_data_20_pre, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))

flipped_post_20 <- glmer(SamplesInAOI / SamplesTotal ~ factor(language, levels = c("Bilinguals", "Monolinguals")) * time_bin_centred * trial_num_scaled + vocab_scaled + vocab_scaled:trial_num_scaled + vocab_scaled:language + (1 | recording_name), data = model_data_20_post, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))
...
# 7-months
# correlations
cor(corr_bi_7$`pre-switch`, corr_bi_7$`post-switch`, method = "pearson")
cor.test(corr_bi_7$`pre-switch`, corr_bi_7$`post-switch`, method = "pearson")

# normality assumptions
shapiro.test(corr_bi_7$`pre-switch`) # 0.02
shapiro.test(corr_bi_7$`post-switch`) # 0.02
ggqqplot(corr_bi_7$`pre-switch`, ylab = "pre-switch")
ggqqplot(corr_bi_7$`post-switch`, ylab = "post-switch")

# visualization
ggscatter(corr_bi_7, x = "post-switch", y = "pre-switch", 
...
# 20-months
# correlations
cor(corr_bi_20$`pre-switch`, corr_bi_20$`post-switch`, method = "pearson")
cor.test(corr_bi_20$`pre-switch`, corr_bi_20$`post-switch`, method = "pearson")

# normality assumptions
shapiro.test(corr_bi_20$`pre-switch`) # 0.24
shapiro.test(corr_bi_20$`post-switch`) # 0.45
ggqqplot(corr_bi_20$`pre-switch`, ylab = "pre-switch")
ggqqplot(corr_bi_20$`post-switch`, ylab = "post-switch")

# visualization
ggscatter(corr_bi_20, x = "post-switch", y = "pre-switch", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "post-switch", ylab = "pre-switch")
...
# 7-months
# correlations
cor(corr_mono_7$`pre-switch`, corr_mono_7$`post-switch`, method = "pearson")
cor.test(corr_mono_7$`pre-switch`, corr_mono_7$`post-switch`, method = "pearson")

# normality assumptions
shapiro.test(corr_mono_7$`pre-switch`) # 0.25
shapiro.test(corr_mono_7$`post-switch`) # 0.000004
ggqqplot(corr_mono_7$`pre-switch`, ylab = "pre-switch")
ggqqplot(corr_mono_7$`post-switch`, ylab = "post-switch")

# visualization
ggscatter(corr_mono_7, x = "post-switch", y = "pre-switch", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "post-switch", ylab = "pre-switch")
...
# 20-months
# correlations
cor(corr_mono_20$`pre-switch`, corr_mono_20$`post-switch`, method = "pearson")
cor.test(corr_mono_20$`pre-switch`, corr_mono_20$`post-switch`, method = "pearson")

# normality assumptions
shapiro.test(corr_mono_20$`pre-switch`) # 0.50
shapiro.test(corr_mono_20$`post-switch`) # 0.03
ggqqplot(corr_mono_20$`pre-switch`, ylab = "pre-switch")
ggqqplot(corr_mono_20$`post-switch`, ylab = "post-switch")

# visualization
ggscatter(corr_mono_20, x = "post-switch", y = "pre-switch", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "post-switch", ylab = "pre-switch")
...
```{r}
#--------------load models
load(here("data", "vis_model_pre.Rda"))
load(here("data", "vis_model_post.Rda"))
load(here("data", "aud_model_pre.Rda"))
load(here("data", "aud_model_post.Rda"))
load(here("data", "dsouza_pre.Rda"))
load(here("data", "dsouza_post.Rda"))

#---------------tables

tab_model(final_model_pre_7, dsouza_pre, vis_model_pre, aud_model_pre, show.re.var = FALSE, show.icc = FALSE)
tab_model(final_model_post_7, dsouza_post, vis_model_post, aud_model_post, show.re.var = FALSE, show.icc = FALSE)
tab_model(final_model_post_20, show.re.var = FALSE, show.icc = FALSE)


#-----------------Make study comparison table

studies_info <- tibble(study_name = c("Study 1a", "Study 1b", "Study 1c", "Study 1d", "Study 2"), original_authors = c("Pour Iliai et al.", "D'Souza et al.", "Kalashnikova et al.", "Kalashnikova et al.", "Pour Iliai et al."), age_range = c("7 months", "7–9 months", "7 months", "7 months", "20 months"), original_n = c(43, 102, 70, 67, 38), original_mo = c(21, 51, 40, 38, 22), original_bi = c(22, 51, 30, 29, 16), analyzed_n = c(43, 53, 41, 44, 38), analyzed_mo = c(21, 29, 25, 25, 22), analyzed_bi = c(22, 24, 16, 19, 16), stimuli = c("Simplified audio-visual cue", "Visual cue with AAB or ABB sequence", "Visual cue with AAB or ABB sequence", "Auditory cue with AAB or ABB sequence", "Simplified audio-visual cue"), population = c("Homogenous sample (French- or English-speaking monolinguals; predominantly French—English bilinguals", "Heterogenous sample (English-speaking monolinguals, English-other bilinguals", "Homogenous sample (Spanish- or Basque-speaking monolinguals, Spanish–Basque bilinguals", "Homogenous sample (Spanish- or Basque-speaking monolinguals, Spanish–Basque bilinguals", "Homogenous sample (French- or English-speaking monolinguals; French—English bilinguals")) %>% mutate_if(is.numeric, funs(as.character(.))) %>% pivot_longer(-1, names_to = "variable", values_to = "value")%>%
    pivot_wider(names_from = "study_name", values_from = "value")
