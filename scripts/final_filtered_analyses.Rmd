...
learning_7_data <- 
  ...
  group_by(recording_name, trial_name, trial_type, language) %>%
  summarize(mean_prop = mean(Prop, na.rm = TRUE),
            SamplesTotal = mean(SamplesTotal, na.rm = TRUE)
            )
...
tab_model(learning_model_7) # fix these to get the results out... 
```
...
# we added one data point to Monolinguals in the post-switch_1, trial 1, in the value of 0.05
# this introduces some variance and the model converge
new_temp <- tibble(recording_name = "fake_p", trial_name = c("pre-switch_9", "post-switch_1"), 
              trial_type = c("pre-switch", "post-switch"), language = "Monolinguals", 
              SamplesTotal = c(12, 12), mean_prop = c(0.05, 0.05))

learning_20_data2 <- 
  bind_rows(learning_20_data, new_temp) %>% 
  mutate(
    trial_type = fct_rev(trial_type),
    language = fct_rev(language)
  )

View(learning_20_data2)

### Visualizations to understand the huge SEs
learning_20_data2 %>% 
...
# model
learning_model_20  <- glmer(mean_prop ~ language * trial_type + (1 | recording_name), 
                            data = learning_20_data2, 
                            family = binomial, 
...
tab_model(learning_model_20) # fix these to get the results out... 
```

#### Try to incldude timebin
