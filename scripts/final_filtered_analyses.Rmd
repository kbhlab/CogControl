...
# try to move the 0s a bit for the monolinguals - post-switch
learning_7_data2 <- 
  learning_7_data %>% 
  mutate(mean_prop = case_when(language == "Monolinguals" & trial_type == "post-switch" & mean_prop == 0 ~ 0.04,
                               TRUE ~ mean_prop))
...
learning_model_7_2  <- glmer(mean_prop ~ language * trial_type + (1 | recording_name), data = learning_7_data2, family = binomial, weights = SamplesTotal, control=glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000))) 

summary(learning_model_7_2) #result is as expected (significantly worse scores at test)
